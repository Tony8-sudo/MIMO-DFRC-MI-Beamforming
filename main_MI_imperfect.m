...
%    comparison between SDR and ZF. It also includes imperfect-angle 
%    Monte-Carlo evaluation for radar MI. To reproduce Figs. 7 in this paper, 
%    please manually adjust the corresponding simulation parameters.
%
...
results_curve.qsdp_Ic_imperfect = zeros(num_gamma,1);
results_curve.sdr_Ic_imperfect  = zeros(num_gamma,1);
results_curve.qsdp_time_avg = zeros(num_gamma,1);
results_curve.sdr_time_avg  = zeros(num_gamma,1);

% Do NOT store R_avg. R is saved sample-by-sample below.
...
Ic_qsdp_imperfect_all = zeros(total_MC_target,num_gamma);
Ic_sdr_imperfect_all  = zeros(total_MC_target,num_gamma);
...
Ic_qsdp_imperfect_dist_avg = zeros(distance_MC,num_gamma);
Ic_sdr_imperfect_dist_avg  = zeros(distance_MC,num_gamma);
...
for dist_idx = 1:distance_MC
    ...
    while dist_valid_count < MC_target && attempt_count_dist < MC_max_attempts
        ...
        Ic_qsdp_imperfect_group = zeros(num_gamma,1);
        Ic_sdr_imperfect_group  = zeros(num_gamma,1);
        ...
        if group_valid
            Ic_qsdp_angle_sum = zeros(num_gamma, 1);
            Ic_sdr_angle_sum  = zeros(num_gamma, 1);

            for angle_idx = 1:angle_MC
                loc_t_delta = loc_t_fixed + (2 * rand(1, K) - 1) * angle_error_bound;
                loc_s_delta = loc_s_fixed + (2 * rand(1, S) - 1) * angle_error_bound;

                A_t_delta = zeros(Nt, K); B_t_delta = zeros(Nr, K);
                for kk = 1:K
                    A_t_delta(:, kk) = steering_ula(loc_t_delta(kk), Nt).';
                    B_t_delta(:, kk) = steering_ula(loc_t_delta(kk), Nr).';
                end
                A_s_delta = zeros(Nt, S); B_s_delta = zeros(Nr, S);
                for ss = 1:S
                    A_s_delta(:, ss) = steering_ula(loc_s_delta(ss), Nt).';
                    B_s_delta(:, ss) = steering_ula(loc_s_delta(ss), Nr).';
                end

                [Rr_delta, ~] = build_target_covariance(A_t_delta, B_t_delta, sigma_0);
                Rsc_delta = build_target_covariance(A_s_delta, B_s_delta, sigma_1);

                for gamma_idx = 1:num_gamma
                    Gamma = Gamma_list(gamma_idx);

                    [~, R_qsdp_delta, ~, status_qsdp_delta, valid_qsdp_delta] = solve_zf_qsdp(A_t_delta, A_s_delta, H, P, Gamma, tao, L, Nt, U, K, S, sigma_u, e, c);
                    [~, R_sdr_delta,  ~, status_sdr_delta,  valid_sdr_delta ] = SDPU3(A_t_delta, A_s_delta, H, P, Gamma, tao, L, Nt, Nr, U, K, S, sigma_u, e, c);

                    if ~(valid_qsdp_delta && valid_sdr_delta)
                        group_valid = false;
                        reject_gamma_count(gamma_idx) = reject_gamma_count(gamma_idx) + 1;
                        fprintf('[Dist %d][Attempt %d/%d][AngleMC=%d/%d][Group Skip][Gamma=%2d dB][Imperfect] status: QSDP=%s | SDR=%s\n', ...
                            dist_idx, attempt_count_dist, MC_max_attempts, angle_idx, angle_MC, ...
                            Gamma_dB_list(gamma_idx), status_qsdp_delta, status_sdr_delta);
                        break;
                    end

                    [Ic_tmp, ~] = compute_mi(Nt, Nr, sigma_r, L, R_qsdp_delta, Rr_delta, Rsc_delta);
                    Ic_qsdp_angle_sum(gamma_idx) = Ic_qsdp_angle_sum(gamma_idx) + Ic_tmp;

                    [Ic_tmp, ~] = compute_mi(Nt, Nr, sigma_r, L, R_sdr_delta, Rr_delta, Rsc_delta);
                    Ic_sdr_angle_sum(gamma_idx) = Ic_sdr_angle_sum(gamma_idx) + Ic_tmp;
                end
            ...
            if group_valid
                Ic_qsdp_imperfect_group = Ic_qsdp_angle_sum / angle_MC;
                Ic_sdr_imperfect_group  = Ic_sdr_angle_sum  / angle_MC;
            end
        end
        ...
        Ic_qsdp_imperfect_all(idx_keep,:) = Ic_qsdp_imperfect_group.';
        Ic_sdr_imperfect_all(idx_keep,:)  = Ic_sdr_imperfect_group.';
        ...
        for gamma_idx = 1:num_gamma
            fprintf('[Dist %d][Attempt %d/%d][Group Keep %d/%d][Gamma=%2d dB] true: QSDP=%.4f, SDR=%.4f | imperfect: QSDP=%.4f, SDR=%.4f | time(s): QSDP=%.4f, SDR=%.4f\n', ...
                ...
                Ic_qsdp_imperfect_group(gamma_idx), Ic_sdr_imperfect_group(gamma_idx), ...
    ...
    Ic_qsdp_imperfect_dist_avg(dist_idx,:) = mean(Ic_qsdp_imperfect_all(dist_sample_idx,:), 1);
    Ic_sdr_imperfect_dist_avg(dist_idx,:)  = mean(Ic_sdr_imperfect_all(dist_sample_idx,:), 1);
...
results_curve.Ic_qsdp_imperfect_dist_avg = Ic_qsdp_imperfect_dist_avg;
results_curve.Ic_sdr_imperfect_dist_avg  = Ic_sdr_imperfect_dist_avg;
...
% Save complete R tensors.
% Dimensions: Nt x Nt x MC_target x distance_MC x num_gamma.
...
for gamma_idx = 1:num_gamma
    ...
    results.qsdp = struct( ...
        ...
        'Ic_imperfect_avg', mean(Ic_qsdp_imperfect_dist_avg(:,gamma_idx)), ...
    ...
    results.sdr = struct( ...
        ...
        'Ic_imperfect_avg', mean(Ic_sdr_imperfect_dist_avg(:,gamma_idx)), ...
    ...
    results_curve.qsdp_Ic_imperfect(gamma_idx) = results.qsdp.Ic_imperfect_avg;
    results_curve.sdr_Ic_imperfect(gamma_idx)  = results.sdr.Ic_imperfect_avg;
